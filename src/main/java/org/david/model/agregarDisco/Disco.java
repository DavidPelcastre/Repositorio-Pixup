...
import org.david.model.sql.implementacion.DiscoJdbcImpl;
...
import static org.david.model.agregarDisco.Artista.artistas;
import static org.david.model.agregarDisco.Disquera.disqueras;
import static org.david.model.agregarDisco.GeneroMusical.generos;
...
public class Disco extends Catalogo {
    ...
    public void setIdDisquera(int id){
        this.disquera = (Disquera) buscarPorID(disqueras, id);
    }
    public int getIdDisquera(){
        return disqueras.indexOf(disquera);
    }
    public void setIdArtista(int id){
        this.artista = (Artista) buscarPorID(artistas, id);
    }
    public int getIdArtista(){
        return artistas.indexOf(artista);
    }
    public void setIdGeneroMusical(int id){
        this.generoMusical = (GeneroMusical) buscarPorID(generos, id);
    }
    public int getIdGeneroMusical(){
        return generos.indexOf(generoMusical) + 1;
    }
    ...
    public void agregarDisco() {
        ...
        Disquera disquera = (Disquera) buscarPorID(disqueras);

        System.out.println("Selecciona un artista:");
        Artista artista = (Artista) buscarPorID(artistas);

        System.out.println("Selecciona un género musical:");
        GeneroMusical generoMusical = (GeneroMusical) buscarPorID(generos);
    ...
    public void leerBaseDatos() {
        discos = DiscoJdbcImpl.getInstance().findAll();
    }
